...
export function AuthProvider({ children }) {
  ...
  const loadProfileForUser = useCallback(
    async (userId, { preferCache } = { preferCache: true }) => {
      if (!userId) return;
  ...
  const resolveSession = useCallback(
    async (
      nextSession,
      { preferCacheForProfile } = { preferCacheForProfile: true },
    ) => {
      ...
      void loadProfileForUser(nextUser.id, {
        preferCache: preferCacheForProfile,
      });
    },
  ...
  useEffect(() => {
    ...
    const {
      ...
      await resolveSession(nextSession, {
        preferCacheForProfile: event === "INITIAL_SESSION",
      });
    });

    return () => {
      mountedRef.current = false;
      subscription?.unsubscribe(); // Crucial para StrictMode
    };
  }, [resolveSession, clearLocalAuthState]);
